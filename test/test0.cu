...
__global__ void gauge_field_grad_kernel(const real_t* __restrict__ fermi,
    ...
    for (int t = 0; t < N; t++) {
        ...
        const int idx_zm = t * N * N * N + ((iz - 1 + N) % N) * N * N + iy * N + ix;       
        const int idx_zp = t * N * N * N + ((iz + 1) % N) * N * N + iy * N + ix;
        gauge_grad[idx] = 0.5 * (gauge[idx_xp] - gauge[idx_xm] +
                                 gauge[idx_yp] - gauge[idx_ym] +
                                 gauge[idx_zp] - gauge[idx_zm]) / lat->Tmp()[idx];
    }
}
